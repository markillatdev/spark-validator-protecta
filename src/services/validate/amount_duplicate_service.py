...
class AmountDuplicateHandler:
    ...
    def procesar_montos(self, systems_validate: List[str], invoiceIds: List[int]):
        ...
        for system in systems_validate:
            ...
            df_facturas_filtradas = df_facturas_filtradas.groupBy("codigo_afiliado", "monto", "ruc_proveedor", "tipo_impuesto").count()
        ...
        for row in duplicados_list:
            ...
            for value in facturas_lists:
                ...
                if estado_id in Constants.ESTADOS_VALIDOS:
                    if cantidad > 1:
                        observation: str = MessageHandler.message_amounts_duplicate(self.message, value, system)
    ...
    def createDataFrameInvoice(self, df_facturas_buscar: DataFrame) -> DataFrame:
        ...
        data = [(row['codigo_afiliado'], row['monto'], row['ruc_proveedor'], row['tipo_impuesto']) for row in query_results]
        schema = StructType([
